...
領域の中央座標をデローネ三角形でポリゴン化したのがこれ。ポイント数が削減されて軽くなった上に、いい感じで面になっています。
...
四分木の領域の中央を採用すると誤差が大きくなりそうなので、

- 四分木領域に２個以下しかポイントがない場合はそれらを採用
- ポイントが３個以上ある場合は領域の中央を採用してポイント数を削減
...
毎回10万行のCSVをダウンロードしてブラウザの中でデータの間引きと補間を実行するのは無駄なので、パラメータ調整用のアプリと、画面表示用アプリを分離した方が良さそう。
