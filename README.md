...
魚群探知機から抽出したGPS座標にそってオブジェクトを移動するなら、この例が使えそう。
...
とりあえず、見た目の美しさは度外視して、パーティクルを風に流されるように移動させてみた。
...
## GPGPU move along wind flow（失敗だけど、まぁいっか）

パーティクルをオフスクリーンにレンダリングしてテクスチャを作成して、１フレーム前のテクスチャと合成するようにしたのがこれ。

せっかくパーティクルに色を付けてるのに、テクスチャの色を単純に加算するとパーティクルの色が白く飛んでしまうので、RGBのいずれかが1.0を超えるようなら2.0で除算するようにしてみた。

当然、これじゃダメなんだけど、いまの実力ではどうしていいか分からない。

`preserveDrawingBuffer` と同じことをフラグメントシェーダーでやりたいだけなんだけど・・・
