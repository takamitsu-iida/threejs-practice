...
このポイントクラウドは明らかにデータが密な場所と疎な場所で差がありすぎるので、地形図にするには工夫が必要です。

まず、(lon, lat)の領域を四分木に分割してデータの密度の高いところ、低いところに分けていきます。
データの密度の高いところはより詳細な領域になるようにします。

領域の中央座標をデローネ三角形でポリゴン化したのがこれ。

ポイント数が削減されて軽くなった上に、いい感じで面を張れています。
...
四分木の領域の中央の座標を採用するのはさすがに乱暴なので、

- 四分木領域に２個以下しかポイントがない場合はそれらを採用
- ポイントが３個以上ある場合は領域の中央座標を採用してポイント数を削減（水深は逆距離加重法で計算）

というルールに変更して頂点間をデローネ三角形でメッシュ化。

さらにtopojsonで三浦市油壷周辺の地図を重ねて、縮尺表示やランドマークを表示してみたのがこれ。

なかなかいい感じに表示できていると思います。
...
> より正確に海底の地形を表示しますが、
> データが存在する近傍しか表示されなくなりますので、
> データのないところは抜けてしまいます。これは期待される動作です。
...
- ボート釣りに行って水深データを収集する
- 四分木の実装をPythonに移して、事前にデータ処理をすませておきたい（JavaScript側は表示に専念したい）
...
> これはまだ作成途中です。
>
> マウスで画面を移動すると、必要に応じてファイルをダウンロードして表示するようにしたいと思っています。
