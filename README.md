...
オブジェクトにとっての水平面がXZ平面であればわりと話しは簡単で、そうでない場合が問題。
たとえばオブジェクトがジェットコースターの場合。
ジェットコースターにとっての上方向というのは、レールの接線に対して垂直方向を指す。
どこを軸にして回転させれば、ジェットコースターは自然な見た目になるのか、
というのをうまく解決してくれるのがクォータニオン。

どの軸に沿って回転させるか、を決めるときに便利なのが行列の外積計算。
レールの接線を表すベクトルと、オブジェクトにとっての上方向のベクトルで外積を取ると互いに直交するベクトルができる。
それを回転軸に使うとよい。
