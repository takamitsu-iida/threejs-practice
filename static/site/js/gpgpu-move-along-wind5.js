...
//
// NOTE: GPGPUで用いているテクスチャデータの中身を変更。
//
  ...
  initComputationRenderer = () => {
    ...
    // テクスチャに情報を埋め込む、
    {
      ...
      for (let i = 0; i < this.params.numParticles; i++) {
        ...
        // 初期状態はランダムな場所に配置する
        positionArray[index + 0] = Math.random() * width - width / 2;     // 位置のX座標
        positionArray[index + 1] = Math.random() * height - height / 2;   // 位置のY座標
        positionArray[index + 2] = 0.0                                    // 速度の絶対値
        positionArray[index + 3] = 0.0;                                   // 何フレーム経過したか
      }
    }
    ...
    const texturePositionShader = /* glsl */`
      ...
      void main() {
        ...
        // 取り出したvec4の使い方は(位置X, 位置Y, 速度絶対値, 経過フレーム数)と定義
        ...
        vec2 windVelocity = getVelocityFromTexture(currentPosition);

        // 新しい位置を計算
        vec2 newPosition = currentPosition + windVelocity * u_particle_speed;
        ...
        // ランダムな場所に飛ばすかどうかのフラグ
        // dropが0.0ならそのままの位置にとどまり、1.0ならランダムな位置に飛ぶ
        float drop = 0.0;

        // texturePositionに保存してある経過フレーム数を取り出す
        float age = texturePositionValue.w;

        // 経過フレーム数を更新
        age += 1.0;

        // 経過フレーム数が90(30fps * 3秒)以上なら、dropの値を確率で1.0にする
        if (age > 90.0) {
          // 速度が早いほど飛びやすくする
          float dropRate = u_drop_rate + length(windVelocity) * u_drop_rate_bump;

          // ランダムに生成した値が1.0-dropRateの範囲に収まっていれば0.0、それ以外は1.0
          // つまりほとんどの場合は0.0で、稀に1.0になる
          drop = step(1.0 - dropRate, rand(newPosition.xy * u_rand_seed));
        }
        ...
        // 新しい位置情報を書き込む
        gl_FragColor = vec4(newPosition, length(windVelocity), age);
      }
    `;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: /*glsl*/`
        ...
        void main() {
          ...
          vVelocity = texturePositionValue.z;
      ...
      fragmentShader: /*glsl*/`
        ...
        void main() {
          ...
          // 0.0 ~ 1.0に正規化した速度を取得
          float speed = clamp(vVelocity, 0.0, 1.0);
