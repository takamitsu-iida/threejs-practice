...
export class Main {
  ...
  quadtree = (area, maxpoints, maxdivision) => {
    ...
    const areas = [
      new Area(area.x1, area.y1, midX, midY, area.depth + 1),
      new Area(midX, area.y1, area.x2, midY, area.depth + 1),
      new Area(area.x1, midY, midX, area.y2, area.depth + 1),
      new Area(midX, midY, area.x2, area.y2, area.depth + 1)
    ];

    for (const point of area.points) {
      for (const subArea of areas) {
        if (point.lat >= subArea.x1 && point.lat < subArea.x2 && point.lon >= subArea.y1 && point.lon < subArea.y2) {
    ...
    let result = [];
    for (const subArea of areas) {
      result = result.concat(quadtree(subArea, maxpoints, maxdivision));
    }
  ...
  dumpAreas = (areas) => {
    for (const area of areas) {
      console.log(`${area.x1} ${area.y1} ${area.x2} ${area.y2}`);
      for (const point of area.points) {
        console.log(point);
      }
    }
  ...
  initQuadTree = () => {
    ...
    const y_max = 139.622;
    const maxpoints = 5;
    const maxdivision = 20;

    // 対象とする領域を生成
    const initial = new Area(x_min, y_min, x_max, y_max, 0);
    for (const d of data) {
      initial.append(d);
    }

    // 分割
    const areas = quadtree(initial, maxpoints, maxdivision);
